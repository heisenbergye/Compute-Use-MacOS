    ...
    with st.sidebar:
        ...
        if st.button("Reset", type="primary"):
            with st.spinner("Resetting..."):
                ...
                subprocess.run("./image/start_all.sh", shell=True)  # noqa: ASYNC221
...
def _tool_output_callback(
    ...
    if tool_output.output:
            print(f"> Tool Output [{tool_id}]:", tool_output.output)
    if tool_output.error:
            print(f"!!! Tool Error [{tool_id}]:", tool_output.error)
    if tool_output.base64_image:
            # Save the image to a file if needed
            os.makedirs("screenshots", exist_ok=True)
            image_data = tool_output.base64_image
            with open(f"screenshots/screenshot_{tool_id}.png", "wb") as f:
                f.write(base64.b64decode(image_data))
            print(f"Took screenshot screenshot_{tool_id}.png")
